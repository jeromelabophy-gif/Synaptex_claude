...
python3 synaptex.py status           # connectivity: git provider + Ollama + index
python3 synaptex.py sync --dry-run   # validate without writing
python3 synaptex.py sync             # sync git provider → memory sheets + index
...
- **`forge.py` multi-git + local**: `GIT_TYPE=local` scans `LOCAL_REPOS_PATH` for `.git/` directories and reads files directly — no network, no token.
